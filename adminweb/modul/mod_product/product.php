...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Product</h2>
          <input type=button value='Tambah Product' onclick=\"window.location.href='?module=product&act=tambahproduct';\">
          <table>
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil = mysql_query("SELECT * FROM product ORDER BY id_product DESC LIMIT $posisi,$batas");
    }
    else{
      $tampil=mysql_query("SELECT * FROM product 
                           WHERE username='$_SESSION[namauser]'       
                           ORDER BY id_product DESC LIMIT $posisi,$batas");
    }
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?module=product&act=editproduct&id=$r[id_product]>Edit</a> | 
		                <a href=$aksi?module=product&act=hapus&id=$r[id_product]>Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      ...
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM product WHERE username='$_SESSION[namauser]'"));
    }  
  ...
  case "tambahproduct":
    ...
    echo "</select></td></tr>
          <tr><td>Isi Product</td>  <td> <textarea name='isi_product' style='width: 450px; height: 350px;'></textarea></td></tr>
  ...
  case "editproduct":
    $edit = mysql_query("SELECT * FROM product WHERE id_product='$_GET[id]'");
    ...
    echo "</select></td></tr>
          <tr><td>Isi Product</td>   <td> <textarea name='isi_product' style='width: 450px; height: 350px;'>$r[isi_product]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> : <img src='../foto_product/small_$r[gambar]'></td></tr>
