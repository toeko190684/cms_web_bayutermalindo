...
switch($_GET[act]){
  ...
  default:
    echo "<h2>profil</h2>
          <input type=button value='Tambah profil' onclick=\"window.location.href='?module=profil&act=tambahprofil';\">
          <table>
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil = mysql_query("SELECT * FROM profil ORDER BY id_profil DESC LIMIT $posisi,$batas");
    }
    else{
      $tampil=mysql_query("SELECT * FROM profil 
                           WHERE username='$_SESSION[namauser]'       
                           ORDER BY id_profil DESC LIMIT $posisi,$batas");
    }
      ...
      echo "<tr><td>$no</td>
                ...
                <td>$tgl_posting</td>
		            <td><a href=?module=profil&act=editprofil&id=$r[id_profil]>Edit</a> | 
		                <a href=$aksi?module=profil&act=hapus&id=$r[id_profil]>Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM profil"));
    }
    else{
      $jmldata = mysql_num_rows(mysql_query("SELECT * FROM profil WHERE username='$_SESSION[namauser]'"));
    }  
  ...
  case "tambahprofil":
    echo "<h2>Tambah profil</h2>
          <form method=POST action='$aksi?module=profil&act=input' enctype='multipart/form-data'>
          <table>
    ...
    echo "</select></td></tr>
          <tr><td>Isi profil</td>  <td> <textarea name='isi_profil' style='width: 450px; height: 350px;'></textarea></td></tr>
  ...
  case "editprofil":
    ...
    echo "</select></td></tr>
          <tr><td>Isi profil</td>   <td> <textarea name='isi_profil' style='width: 450px; height: 350px;'>$r[isi_profil]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> : <img src='../foto_profil/small_$r[gambar]'></td></tr>
